...
def main(callbacks=Callbacks()):
    opt = load_config('config.yaml')  # Load the configuration into opt
    
    print("Configuration loaded for training:")
    print(opt)  # This will display all loaded configurations as before

    # Example of using opt in the script
    print(f"Training will run for {opt.epochs} epochs with a batch size of {opt.batch_size}.")
    # Checks
    if RANK in {-1, 0}:
        print_args(vars(opt))
        #check_git_status()
...
def load_config(config_file):
    ...
    # Convert the dictionary to a Namespace for easier attribute access
